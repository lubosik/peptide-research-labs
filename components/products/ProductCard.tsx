...
export default function ProductCard({ product, className = '', isDiscontinued = false, airtableInStock }: ProductCardProps) {
  ...
  return (
    <div
      className={`bg-secondary-charcoal rounded-lg border border-luxury-gold/20 overflow-hidden shadow-md transition-all duration-400 flex flex-col ${className} ${isDiscontinued ? 'opacity-50 pointer-events-none' : ''}`}
      style={{
    ...
    >
      ...
      {!inStock && !isDiscontinued && (
        <div className="absolute top-4 right-4 z-10 bg-[#E74C3C] text-white font-bold px-3 py-1 rounded-md text-sm shadow-lg">
          OUT OF STOCK
        </div>
      )}
